...
public class InsertStoreDao implements StoreDao {
	...
	/*
	 * @Override public void persist(String propertiesPath, StoreDto storeToPersist)
	 ...
	 * postgresConnector.startDatabaseConnection(propertiesPath); PreparedStatement
	 * preparedStatementStore = postgresConnection
	 ...
	 * 
	 * storeToPersist.getProducts().forEach(product -> { try (PreparedStatement
	 * preparedStatementProduct = postgresConnection
	 ...
	 * 
	 * preparedStatementInventory.setInt(1, existingProductStore.getInt("amount") +
	 * product.getInventory().getAmount()); preparedStatementInventory.setInt(2,
	 * existingProductStore.getInt("id"));
	 * 
	 * if (preparedStatementInventory.executeUpdate() == 1) {
	 * log.info("Inventory updated"); } } } else { try (PreparedStatement
	 * preparedStatementProductDatabase = postgresConnection
	 * .prepareStatement(Queries.CHECK_EXISTING_PRODUCT_DATABASE.getQuery())) {
	 * 
	 * preparedStatementProductDatabase.setInt(1, product.getId());
	 * 
	 * ResultSet existingProductDatabase =
	 * preparedStatementProductDatabase.executeQuery();
	 * 
	 ...
	 * 
	 * persistInventory.setInt(1, product.getId()); persistInventory.setInt(2,
	 * product.getInventory().getStoreId()); persistInventory.setInt(3,
	 * product.getInventory().getAmount()); persistInventory.setBigDecimal(4,
	 * product.getInventory().getPrice());
	 * 
	 * if (persistInventory.executeUpdate() == 1) { log.info("Inventory persisted");
	 * } } } else { try (PreparedStatement persistProduct = postgresConnection
	 ...
	 * (SQLException e) { log.error(e.getMessage(), e); } }); } else {
	 ...
	 * 
	 * switch (preparedStatementInsertStore.executeUpdate()) { case 1:
	 ...
	 * 
	 * if (persistInventory.executeUpdate() == 1) { log.info("Inventory persisted");
	 * } } } } } } catch (SQLException e) { e.printStackTrace(); } }); break; case
	 * 2: log.info("Could not persist this store"); throw new Exception(); } } }
	 * 
	 * } catch (SQLException e) { log.error(e.getMessage(), e); throw new
	 * StoreDaoException(ErrorMessages.SQL_INSERT_STORE_DAO_ERROR); } catch
	 * (Exception e) { log.error(e.getMessage(), e); throw new
	 * StoreDaoException(ErrorMessages.INSERT_STORE_DAO_ERROR); } }
	 */
}
